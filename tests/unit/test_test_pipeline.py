...
def test_load_model_success(tmp_path):
    models_dir = tmp_path
    model_path = models_dir / "model.pkl"  # nome compatível com glob
    model_path.write_bytes(b"fake")  # arquivo fake

    # patch correto do joblib
    with patch("src.train_test.test.joblib.load") as mock_load:
        mock_load.return_value = "mock_model"
        model, timestamp = load_model(models_dir)

    assert model == "mock_model"
    # timestamp pega a última parte do nome do arquivo; aqui é "model"
    assert timestamp == "model"
